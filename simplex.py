...
class SimplexSolver:
    ...
    def run_simplex(self, A, b, c, prob='max', ineq=[],
                    enable_msg=False, latex_path=None):
        """ Run simplex algorithm.
        """
        if latex_path is not None and len(latex_path) > 0:
            latex = True
        else:
            latex = False

        if prob == 'min':
            raise Exception('Minimisation problem does not work.')
    ...
    def print_doc(self):
        ...
        if not self.latex_path.endswith('.tex'):
            self.latex_path += '.tex'

        with open(self.latex_path, 'w') as tex:
            tex.write(self.doc)
